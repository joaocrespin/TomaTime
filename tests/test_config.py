...
def test_missing_file_load_config(config_path: Path) -> None:
    ...
    assert result == {"focus_time": 25,"break_time": 5,}
    assert config_path.exists()

def test_existing_file_load_config(config_path: Path) -> None:
    config_path.parent.mkdir(parents=True)
    config_path.write_text(json.dumps({"focus_time": 50, "break_time": 10}))
    config = Config(file_path=config_path)
    result = config.load_config()
    assert result == {"focus_time": 50, "break_time": 10}

def test_set_config(config_path: Path) -> None:
    config = Config(file_path=config_path)
    config.load_config()
    config.set_config(30, 7)
    result = config.load_config()
    assert result == {"focus_time": 30, "break_time": 7}
